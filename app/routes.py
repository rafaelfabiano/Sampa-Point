...
def link_create_user():
    ...
    # Renderiza o dashboard para a Agência de Franquias
    return render_template("dashboard.html", role="agencia")


    send_notification = os.getenv('SEND_NOTIFICATION')  # Carrega a variável do .env
    print(f"SEND_NOTIFICATION: {send_notification}")  # Verifica no log se o valor está correto
    return redirect(send_notification)  # Redireciona para o link armazenado